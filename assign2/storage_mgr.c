...
#ifdef __DEBUG__
        printf("\n[fileHandle] fileName: %s, totalNumPages: %d, getBlockPos: %d", fileHandle->fileName, fileHandle->totalNumPages, getBlockPos(fileHandle));
        printf("\n");
        printf("\n readBlock");
#endif
...
#ifdef __DEBUG__
    printf("\n[readBlock] pageNum: %d, curPagePos: %d, totalNumPages: %d, memPage: %s", pageNum, fileHandle->curPagePos, fileHandle->totalNumPages, memPage);
#endif
    return RC_OK;
}
...
#endif
    if (fseek(fp, pageLocation, SEEK_SET) != 0)
        return RC_WRITE_FAILED;
    // creating a block
